...
class TokenCandidate:
    ...
    def is_early_entry(self) -> bool:
        """Check if we're not buying mid-pump"""
        # Avoid anything already pumped 40%+ in 5 min
        if self.price_change_5m > 40:
            return False
        # Avoid tokens older than 24 hours for meme plays
    ...
    def summary(self) -> str:
        ...
        return (
            f"{self.symbol} ({self.chain.upper()})\n"
...
class Scanner:
    ...
    def _passes_filters(self, c) -> bool:
        """Enhanced filters with entry timing and buy pressure"""
    ...
    async def _process_candidate(self, candidate):
        logger.info(
            "Processing: %s | +%.1f%% | Buy pressure: %.0f%% | Age: %.1fh",
            candidate.symbol, candidate.price_change_5m,
            candidate.buy_pressure(), candidate.age_hours
        )
        ...
        if trade and trade.success:
            ...
            await self._telegram.send_trade_failed(
                candidate, trade.error if trade else "unknown"
            )
